...
export default function RankingsTable({ initialGlobal, initialCountries, locale }: Props) {
  ...
  useEffect(() => {
    if (tab === 'global') { setRows(initialGlobal); return; }
    if (!country) { setRows([]); return; }
    setLoading(true);
    fetch(`/api/rankings/local/${encodeURIComponent(country)}?limit=50`)
      .then(r => r.json())
      .then(d => setRows(d?.data?.rankings || []))
      .finally(() => setLoading(false));
  }, [tab, country, initialGlobal]);
      ...
      <div className="glass rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[760px]">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={7} className="px-4 py-12 text-center text-text-tertiary">…</td></tr>
              ) : rows.length === 0 ? (
                  ...
                  <td className="px-4 py-3 hidden sm:table-cell">
                    {p.selectedSnake && (
                      <div className="size-12 rounded-lg bg-bg-subtle flex items-center justify-center overflow-hidden">
                        <img src={snakeImg(p.selectedSnake)} alt={p.selectedSnake} loading="lazy" className="w-4/5 h-4/5 object-contain" />
                      </div>
                    )}
                  </td>
