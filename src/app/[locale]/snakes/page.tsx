...
export default async function SnakesPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const country = all.filter(s => s.isCountry);
  const fancy = all.filter(s => !s.isCountry && s.id.startsWith('FSNAKE_')).sort((a, b) => {
    const na = parseInt(a.id.replace('FSNAKE_', ''), 10);
    const nb = parseInt(b.id.replace('FSNAKE_', ''), 10);
    return na - nb;
  });
  const other = all.filter(s => !s.isCountry && !s.id.startsWith('FSNAKE_'));
  ...
  function SkinCard({ skin, dense }: { skin: Skin; dense?: boolean }) {
    return (
      <Link
        ...
        className={`card !p-3 group relative overflow-hidden block focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 focus-visible:ring-offset-bg ${dense ? '!p-1.5' : ''}`}
        aria-label={`${skin.name} — ${tSkin(`rarity.${skin.rarity}`)}`}
      >
        <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity bg-gradient-to-b from-brand-500/10 via-transparent to-magenta-500/10 pointer-events-none ${dense ? 'hidden' : ''}`} aria-hidden="true" />
